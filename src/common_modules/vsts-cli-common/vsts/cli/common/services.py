...
from azdos.customer_intelligence.v4_0.models.customer_intelligence_event import CustomerIntelligenceEvent
from azdos.vss_connection import VssConnection
from .arguments import should_detect
from .config import azdos_config
from ._credentials import get_credential
from .git import get_remote_url
from .version import VERSION
from .azdos_git_url_info import AzdosGitUrlInfo
...
def get_vss_connection(team_instance):
    if team_instance not in _vss_connection:
        ...
        try:
            ...
            if azdos_config.has_option('core', 'collect_telemetry'):
                collect_telemetry = azdos_config.get('core', 'collect_telemetry')
...
def _get_vss_connection(team_instance, credentials):
    return VssConnection(get_base_url(team_instance), creds=credentials, user_agent='azdoscli/{}'.format(VERSION))
...
def get_build_client(team_instance=None):
    ...
    return connection.get_client('azdos.build.v4_0.build_client.BuildClient')


def get_ci_client(team_instance=None):
    connection = get_vss_connection(team_instance)
    return connection.get_client(
        'azdos.customer_intelligence.v4_0.customer_intelligence_client.CustomerIntelligenceClient')


def get_core_client(team_instance=None):
    connection = get_vss_connection(team_instance)
    return connection.get_client('azdos.core.v4_0.core_client.CoreClient')


def get_git_client(team_instance=None):
    connection = get_vss_connection(team_instance)
    return connection.get_client('azdos.git.v4_0.git_client.GitClient')


def get_identity_client(team_instance=None):
    connection = get_vss_connection(team_instance)
    return connection.get_client('azdos.identity.v4_0.identity_client.IdentityClient')


def get_location_client(team_instance=None):
    connection = get_vss_connection(team_instance)
    return connection.get_client('azdos.location.v4_0.location_client.LocationClient')


def get_member_entitlement_management_client(team_instance=None):
    connection = get_vss_connection(team_instance)
    return connection.get_client('azdos.member_entitlement_management.v4_1.member_entitlement_management_client.MemberEntitlementManagementClient')


def get_operations_client(team_instance=None):
    connection = get_vss_connection(team_instance)
    return connection.get_client('azdos.operations.v4_0.operations_client.OperationsClient')


def get_policy_client(team_instance=None):
    connection = get_vss_connection(team_instance)
    return connection.get_client('azdos.policy.v4_0.policy_client.PolicyClient')


def get_settings_client(team_instance=None):
    connection = get_vss_connection(team_instance)
    return connection.get_client('azdos.settings.v4_0.settings_client.SettingsClient')


def get_task_agent_client(team_instance=None):
    connection = get_vss_connection(team_instance)
    return connection.get_client('azdos.task_agent.v4_0.task_agent_client.TaskAgentClient')


def get_work_item_tracking_client(team_instance=None):
    connection = get_vss_connection(team_instance)
    return connection.get_client('azdos.work_item_tracking.v4_0.work_item_tracking_client.WorkItemTrackingClient')
...
def _raise_team_team_instance_arg_error():
    raise CLIError('--instance must be specified. The value should be the URI of your VSTS account, ' +
                   ...
                   'You can set a default value by running: azdos configure --defaults ' +
...
def _raise_team_project_arg_error():
    raise CLIError('--project must be specified. The value should be the ID or name of a team project. ' +
                   'You can set a default value by running: azdos configure --defaults project=<ProjectName>.')
...
def resolve_instance_project_and_repo(detect, team_instance, project=None, project_required=True, repo=None):
    if team_instance is None:
        if should_detect(detect):
            git_info = get_azdos_info_from_current_remote_url()
...
def _resolve_instance_from_config(team_instance):
    from .config import azdos_config
    if team_instance is None:
        if azdos_config.has_option(_DEFAULTS_SECTION, _TEAM_INSTANCE_DEFAULT):
            team_instance = azdos_config.get(_DEFAULTS_SECTION, _TEAM_INSTANCE_DEFAULT)
...
def _resolve_project_from_config(project, project_required=True):
    from .config import azdos_config
    if project is None:
        if azdos_config.has_option(_DEFAULTS_SECTION, _TEAM_PROJECT_DEFAULT):
            project = azdos_config.get(_DEFAULTS_SECTION, _TEAM_PROJECT_DEFAULT)
        if project_required and (project is None or project == ''):
            _raise_team_project_arg_error()
    return project


def get_azdos_info_from_current_remote_url():
    start = datetime.datetime.now()
    info = AzdosGitUrlInfo(get_remote_url(AzdosGitUrlInfo.is_azdos_url_candidate))
...
def set_tracking_data(argv):
    try:
        azdos_tracking_data.area = 'CLI'
        azdos_tracking_data.properties = {}
        if argv is not None and argv:
            azdos_tracking_data.feature = argv[0]
            if len(argv) > 1:
                command = []
                args = []
                ...
                if command:
                    azdos_tracking_data.properties['Command'] = ' '.join(command)
                else:
                    azdos_tracking_data.properties['Command'] = ''
                azdos_tracking_data.properties['Args'] = args
        else:
            azdos_tracking_data.feature = 'Command'
    except Exception as ex:
        logger.debug(ex, exc_info=True)


def _try_send_tracking_ci_event_async(team_instance=None):
    if azdos_tracking_data is not None and azdos_tracking_data.area is not None:
        try:
...
def _send_tracking_ci_event(team_instance=None, ci_client=None):
    ...
    try:
        ci_client.publish_events([azdos_tracking_data])
        return True
    except Exception as ex:
        logger.debug(ex, exc_info=True)
        return False
...
def get_authentication_error(message):
    return CLIError(str(message) + "  Please see https://aka.ms/azdos-cli-auth for more information.")
...
azdos_tracking_data = CustomerIntelligenceEvent()
