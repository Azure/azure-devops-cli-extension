...
def create_work_item(work_item_type, title, description=None, parent=None, assigned_to=None,
    ...
    try:
        ...
        if parent is not None:
            patch_document.append(_create_work_item_relation_patch_operation('parent', parent, 'add', team_instance))
            ...
            for relation in relations:
                ...
                if len(kvp) == 2:
                    for value in kvp[1].split(','):
                        patch_document.append(_create_work_item_relation_patch_operation(kvp[0],
                            value, 'add', team_instance))
                else:
    ...
    try:
        ...
        if indexes is not None:
            for index in indexes.split(','):
                patch_document.append(_create_patch_operation('remove', '/relations/' + index))
        client = get_work_item_tracking_client(team_instance)
        return client.update_work_item(document=patch_document, id=work_item_id).relations
    except Exception as ex:
        handle_command_exception(ex)
...
def _create_work_item_relation_patch_operation(field, value, op, team_instance):
    relation = WorkItemRelation(
        rel=_RELATION_FIELD_ARGS[field],
        url=team_instance + '/apis/wit/workItems/' + value)
    return _create_patch_operation(op=op, path='/relations/-', value=relation)


def _create_patch_operation(op, path, value=None):
