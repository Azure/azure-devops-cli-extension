...
def list_pull_requests(repository=None, creator=None, include_links=False, reviewer=None,
    ...
    :type top: int
    :param team_instance: URI for the Visual Studio Team Services Instance.
    :type team_instance: str
    :param detect: When 'On' unsupplied arg values will be detected from the current working
                   directory's repo.
    :type detect: str
...
def _resolve_reviewers_as_refs(reviewers, team_instance):
    ...
    :param team_instance: The VSTS team_instance URI.
    :type team_instance: str
    :rtype: list of :class:`IdentityRefWithVote
     <git.models.IdentityRefWithVote>`
    """
...
def _open_pull_request(pull_request, team_instance):
    ...
    url = urllib.parse.urljoin(get_base_url(team_instance),
                               urllib.parse.quote(pull_request.repository.project.name)
