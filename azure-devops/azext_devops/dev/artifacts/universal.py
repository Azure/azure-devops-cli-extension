...
def download_package(feed, name, version, path, filter=None, organization=None, detect=None):
    ...
    organization = resolve_instance(detect=detect, organization=organization)
    artifact_tool = ArtifactToolInvoker(ProgressReportingExternalToolInvoker(), ArtifactToolUpdater())
    return artifact_tool.download_universal(organization, feed, name, version, path, filter)
