...
class TestWorkItemTagMethods(AuthenticatedTests):
    ...
    _TEST_WORK_ITEM_NEW_TAG = "new tag"
    ...
    def test_add_work_item_tag_correct_id(self):
        # set return values
        self.mock_update_WI.return_value.id = self._TEST_WORK_ITEM_ID
        self.mock_update_WI.return_value.fields = { "System.Tags": self._TEST_WORK_ITEM_NEW_TAG }

        response = add_work_item_tags(id=self._TEST_WORK_ITEM_ID, tag=self._TEST_WORK_ITEM_NEW_TAG, organization=self._TEST_DEVOPS_ORGANIZATION)

        # assert
        self.mock_validate_token.assert_not_called()
        self.mock_update_WI.assert_called_once_with(document=ANY, id=self._TEST_WORK_ITEM_ID)
        args, kwargs = self.mock_update_WI.call_args
        self.assertEqual(response.id, self._TEST_WORK_ITEM_ID)
        ...
        self.assertEqual(self._TEST_WORK_ITEM_NEW_TAG, actual_patch_document.value)


    def test_add_work_item_tag_none_id(self):
        test_work_item_id = None
        with self.assertRaises(CLIError) as exc:
            add_work_item_tags(id=test_work_item_id, tag=self._TEST_WORK_ITEM_NEW_TAG, organization=self._TEST_DEVOPS_ORGANIZATION)
    ...
    def test_add_work_item_tag_none_tag(self):
        ...
        with self.assertRaises(CLIError) as exc:
            add_work_item_tags(id=self._TEST_WORK_ITEM_ID, tag=test_work_item_new_tag, organization=self._TEST_DEVOPS_ORGANIZATION)
        self.assertIn('--tag must be provided', str(exc.exception))


    def test_list_work_item_tag_correct_id(self):        
        # set return values
        self.mock_get_WI.return_value.id = self._TEST_WORK_ITEM_ID
        self.mock_get_WI.return_value.fields = { "System.Tags": "mytag" }

        response = list_work_item_tags(id=self._TEST_WORK_ITEM_ID, organization=self._TEST_DEVOPS_ORGANIZATION)

        # assert
        self.mock_validate_token.assert_not_called()
        self.mock_get_WI.assert_called_once_with(self._TEST_WORK_ITEM_ID, as_of=None, fields=['System.Id','System.Tags'], expand=None)
        self.assertEqual(response.id, self._TEST_WORK_ITEM_ID)
    ...
    def test_remove_work_item_tag(self):
        ...
        self.mock_update_WI.return_value.id = self._TEST_WORK_ITEM_ID
        self.mock_update_WI.return_value.fields = { "System.Id": self._TEST_WORK_ITEM_ID }

        response = remove_work_item_tags(id=self._TEST_WORK_ITEM_ID, tag=test_work_item_tag_to_remove, organization=self._TEST_DEVOPS_ORGANIZATION)
        ...
        self.assertEqual("/fields/System.Tags", actual_patch_document.path)
        self.assertEqual("", actual_patch_document.value)
    ...
    def test_remove_work_item_nonexistent_tag_from_workitem_with_tags(self):
        ...
        with self.assertRaises(CLIError) as exc:
            remove_work_item_tags(id=self._TEST_WORK_ITEM_ID, tag=test_work_item_tag_to_remove, organization=self._TEST_DEVOPS_ORGANIZATION)
        self.assertIn(f'Work item tag {test_work_item_tag_to_remove} was not found on work item with id {self._TEST_WORK_ITEM_ID}.', str(exc.exception))


    def test_remove_work_item_tag_when_no_tags_exist(self):
        # set return values
        self.mock_get_WI.return_value.id = self._TEST_WORK_ITEM_ID
        self.mock_get_WI.return_value.fields = { "System.Id": self._TEST_WORK_ITEM_ID }
        self.mock_update_WI.return_value.id = self._TEST_WORK_ITEM_ID
        self.mock_update_WI.return_value.fields = { "System.Id": self._TEST_WORK_ITEM_ID }

        with self.assertRaises(CLIError) as exc:
            remove_work_item_tags(id=self._TEST_WORK_ITEM_ID, tag="tag does not exist", organization=self._TEST_DEVOPS_ORGANIZATION)
        self.assertIn(f'Work item with id {self._TEST_WORK_ITEM_ID} has no tags.', str(exc.exception))


    def test_remove_work_item_tag_none_id(self):
        with self.assertRaises(CLIError) as exc:
            remove_work_item_tags(id=None, tag=self._TEST_WORK_ITEM_NEW_TAG, organization=self._TEST_DEVOPS_ORGANIZATION)
        self.assertIn('--id must be provided', str(exc.exception))


    def test_remove_work_item_tag_none_tag(self):
        with self.assertRaises(CLIError) as exc:
            remove_work_item_tags(id=self._TEST_WORK_ITEM_ID, tag=None, organization=self._TEST_DEVOPS_ORGANIZATION)
        self.assertIn('--tag must be provided', str(exc.exception))
