...
class TestCredentialsMethods(unittest.TestCase):
    ...
    def tearDownClass(self):
        ...
        if self._ORIGINAL_DEFAULT_TOKEN is not None:
                ...
                set_credential(devops_organization, self._ORIGINAL_DEFAULT_TOKEN)
    ...
    def test_organization_list_file(self):
        ...
        devops_organization1 = 'https://' + str(uuid.uuid4()) + '.visualstudio.com/'
        token1 = str(uuid.uuid4())
        set_credential(devops_organization1, token1)
        retrieved_token = get_credential(devops_organization1, fall_back_to_default=False)
        self.assertEqual(token1, retrieved_token,)

        devops_organization2 = 'https://' + str(uuid.uuid4()) + '.visualstudio.com/'
        token2 = str(uuid.uuid4())
        set_credential(devops_organization2, token2)
        retrieved_token = get_credential(devops_organization2, fall_back_to_default=False)
        self.assertEqual(token2, retrieved_token,)

        devops_organization3 = 'https://' + str(uuid.uuid4()) + '.visualstudio.com/'
        token3 = str(uuid.uuid4())
        set_credential(devops_organization3, token3)
        retrieved_token = get_credential(devops_organization3, fall_back_to_default=False)
        ...
        self.assertEqual(token1, retrieved_token)
        self.assertTrue(os.path.isfile(ORGANIZATION_LIST_FILE))
        clear_credential(devops_organization1)
        retrieved_token = get_credential(devops_organization1, fall_back_to_default=False)
        self.assertEqual(None, retrieved_token)
