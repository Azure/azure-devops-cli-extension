...
class DevopsTeamTests(ScenarioTest):
    ...
    @disable_telemetry
    def test_devops_team_createUpdateShowListDelete(self):
        ...
        try:
            ...
            create_team_output = self.cmd(create_team_command).get_output_in_json()
            assert len(create_team_output) > 0
            assert create_team_output["name"] == team_name2
            assert create_team_output["description"] == team_description
            ...
            assert len(list_teams_output) > 1
            ...
            #ToDo [ishitam] : Add test for 'list-member' command

        finally:
            #TestCleanup - delete team
            delete_team_command = 'az devops team delete --id "' + team_name + '" --output json --detect off --yes'
