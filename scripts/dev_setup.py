...
os.environ['AZURE_EXTENSION_DIR'] = os.path.join(azure_dir,'cliextensions')

exec_command('pip install -e {}'.format(extension_dir))
exec_command('pip install --upgrade --target {0}/cliextensions/azure-devops {1} --no-user'.format(azure_dir, extension_dir))
