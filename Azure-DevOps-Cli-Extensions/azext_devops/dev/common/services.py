...
def _get_credentials(team_instance):
    ...
    raise_authentication_error('Before you can run VSTS commands, you need to run the login command (az login if org is AAD backed else az devops login) to setup credentials.')
